...
- For logic that does not require After Effects, add dependency-light tests with minimal host mocks for one-based collections, `instanceof`-style item categories, naming rules, classification, and repeat-run behavior. Do not introduce a runtime test dependency into the panel.
- Manually smoke-test host and UI behavior in After Effects: panel launch/docking/resizing, selection validation, folder and comp reuse, sorting and hierarchy preservation, labels with the default palette, repeat runs, undo, and error paths.
